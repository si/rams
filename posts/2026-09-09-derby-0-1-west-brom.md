# Draft: Derby County 0-1 West Bromwich Albion

- **Published:** not yet — drafted, pending the gaps flagged below before it
  goes up on Patreon.
- **Fixture:** EFL Championship, Pride Park Stadium, Wednesday 9 September
  2026, KO 19:45.
- **Status:** Drafted against the `patreon-match-post` skill format. Same
  constraint as every prior draft this season — this session's `WebFetch` is
  blocked for every match-report domain tried (dcfc.co.uk, Sky Sports,
  Flashscore, Sofascore), so everything below comes from `WebSearch`
  snippets rather than a page Claude could read directly. Confidence is
  lower than usual on subs, bookings beyond the red card, attendance, and
  the next fixture — flagged inline and in "Notes for whoever publishes
  this."

---

## Title field

```
Derby 0-1 West Brom: ten men, one goal, and we still couldn't find a way through
```

## Public preview (above the paywall)

```
Sixteen minutes in and West Brom were already ahead through Aune Heggebø — and it barely got better from there, even after Brayann Pereira saw a second yellow midway through the first half and left the visitors down to ten men for nearly an hour. Eustace threw on the extra striker, we threw everything else at it, and West Brom just... defended, and went top of the Championship for their trouble. Full breakdown for patrons below 👇
```

## Patrons-only body (below the paywall)

```
THE MATCH

Derby County 0-1 West Bromwich Albion — EFL Championship
📍 Pride Park Stadium, Derby
🗓️ Wednesday 9 September 2026, KO 19:45


STARTING XI — THE RAMS

⚪ #1 — Jacob Widell Zetterström
⚪ #6 — Sondre Langås
⚪ #28 — Dion Sanderson
⚪ #44 — Eiran Cashin
⚪ #18 — Charlie Taylor
⚪ #20 — Alex Mowatt
⚪ #4 — David Ozoh
⚪ #42 — Bobby Clark
⚪ #11 — Sam Szmodics
⚪ #26 — Ryan Hedges
⚪ #TBC — Mohammed Fuseini


SUBS — WHO CAME OFF, WHO CAME ON

**~65'** Ozoh ↓ · ↑ TBC (extra striker, per Eustace's post-match comments — name not confirmed)


THE PEOPLE, NOT JUST THE SHEET

David Ozoh
An hour of graft in a losing cause, and even his own manager led with it postmatch: "I thought David was excellent for an hour" before the tactical change. When the manager's using you as the headline in a defeat, that's not nothing.

Sam Szmodics
Caused West Brom's back line early problems and looked the closest thing Derby had to a spark — worth watching whether that continues now Eustace clearly trusts him in behind the striker.

Charlie Taylor
The Pereira red card came from a challenge on him — for once, a Derby player benefiting from someone else's discipline rather than being on the wrong end of it. A rare good news story from a game with not many.

The rest of the XI
Ten men for nearly an hour and still no goal to show for it. That's not one player's problem — that's an entire attacking unit that couldn't find the pass or the shot that mattered when the numbers were finally in our favour.


STATS SNAPSHOT

Goals: 16' Heggebø (West Brom, assist Jimmy-Jay Morgan)
Bookings: 34' Brayann Pereira (West Brom) — second yellow, straight red, foul on Charlie Taylor
Attendance: not confirmed in available sources — flag for whoever publishes to check dcfc.co.uk/official EFL figures


THE READ

Losing to ten men for closer to an hour than not is the kind of result that should genuinely worry a manager, not just a fanbase. Eustace made the right call throwing on an extra striker and got an hour of real endeavour out of Ozoh before he did it — but "we tried" isn't a result, and West Brom went top of the Championship playing with a man down for most of the second half. Fair play to their back line; less fair play to ours for not finding a way past it.


WHAT'S NEXT

**Saturday 13 September, Pride Park: Birmingham City (h), Championship.**


IN THEIR WORDS 🗣️

"I'm afraid that was truly embarrassing. I'm not sure what's worse - losing heavily, as #dcfc did against Swansea and West Ham, or being managed without any trouble whatsoever by ten men. West Brom won't get many easier away wins than that. Derby were pathetic. Wrong team, wrong subs. Belief drained." — @derbycountyblog.bsky.social

"Sanderson at RB says it all tonight. Whether fans like him or not, Johnston needs to start the next 5-6 games and give him chance to play his way into form. Unless you're one of Eustace's mates, you won't be given a run of games. It's all chop and change each and every game." — @iamzavagno.bsky.social

"FULL TIME. Derby 0-1 West Brom. Derby may have played against 10 men for most of the game but did nothing to capitalise and are subjected to a deserved defeat. West Brom were excellent even after going down to 10 and deserve their victory." — @worksbyadam.com


LISTEN: RAMS REVIEW PODCAST

The lads broke this one down too — full episode here: https://www.ramsreviewpodcast.com/post/dcfc-0-1-wba-9-sep-26
```

---

## Notes for whoever publishes this

- **This is a genuinely thin data pull, more than usual.** Direct fetches
  to dcfc.co.uk, Sky Sports, Flashscore and Sofascore were all blocked in
  this session — same constraint as the Cardiff draft, but this time it
  bit harder because it's a midweek game with less search coverage. Please
  verify against a live source before publishing, more than the usual
  once-over.
- **Fuseini's squad number is unconfirmed** — marked `#TBC` rather than
  guessed. Everyone else's number matches what's already verified in the
  Cardiff/Rotherham/Lincoln posts, plus Cashin (#44), Ozoh (#4), Szmodics
  (#11) and Hedges (#26) newly sourced this round — but none of those four
  were independently cross-checked against a second source, so give them a
  once-over too.
- **The player who replaced Ozoh is not named in anything search surfaced.**
  Multiple sources confirm Eustace "threw on an extra striker" for Ozoh
  "midway in the second half," but never name who. Fill in before
  publishing — likely Fuseini shifting or a striker off the bench (Morris?
  Carlton Morris didn't appear in this XI per the sources found, so he's
  the obvious candidate, but that's a guess, not a finding).
- **No other Derby substitutions turned up at all**, which is unusual for a
  full 90 minutes — probably just a gap in what search surfaced rather than
  Eustace only making one change. Worth a direct check.
- **Attendance figure never turned up** in any search result, despite
  several attempts. Given West Brom went top and this was a live TV game,
  it's likely to have been well-attended — check dcfc.co.uk or the EFL site
  directly rather than leaving this blank.
- **Next fixture (Birmingham, h, Sat 13 Sept) comes from a fan's Bluesky
  reply**, not an independently checked source — search itself kept
  surfacing old-season and hallucinated fixture noise. Worth a quick
  dcfc.co.uk cross-check before publishing, but a fan naming the exact next
  opponent and day the night of the game is decent evidence on its own.
- **"Charlie Taylor" needs a sanity check.** Some sources returned a Charlie
  Taylor playing for West Brom, contradicting his place in Derby's own
  lineup from the Cardiff post. Almost certainly search conflating a
  same-named player from elsewhere in football — but confirm he's still
  Derby's before this goes up, given he's also the player at the centre of
  the sending-off.
- **Fan quotes are handle-only**, same as the Cardiff post — Si supplied
  screenshots with handles (@derbycountyblog.bsky.social,
  @iamzavagno.bsky.social, @worksbyadam.com) but no display names beyond
  what's shown on-screen, so credited by handle. Trimmed the `#dcfc #efl
  #derbycounty #wwrams`-style hashtag clutter off the ends of two quotes,
  left the wording otherwise verbatim. Si also offered to crop the original
  screenshots for full visual credit if that's preferred over text pulls —
  worth asking before publishing.
- **Podcast plug is unverified content-wise** — `WebFetch` is blocked for
  ramsreviewpodcast.com in this session (same as every prior post), so the
  episode's actual content beyond the title/URL Si gave couldn't be
  confirmed. Listed as a plain link rather than an embed — check on paste
  whether Patreon renders it as a clickable link or just plain text.
- No image yet — same Canva-reliability caveat as every prior post.
- Once published, replace this file's front-matter with the real Patreon
  URL and status, matching the established pattern.

## Sources

- [Derby 0-1 West Brom: Baggies move top of Championship with win at Pride Park despite Brayann Pereira red card (Sky Sports)](https://www.skysports.com/football/derby-county-vs-west-bromwich-albion/report/560567)
- [West Brom win with 10 men at Derby to reach Championship summit (Shropshire Star)](https://www.shropshirestar.com/uk-sports/west-brom-win-with-10-men-at-derby-to-reach-championship-summit-9004421)
- [Derby 0 West Brom 1: Baggies claim brilliant win with 10 men (Express & Star)](https://www.expressandstar.com/sport/derby-0-west-brom-1-baggies-claim-brilliant-win-with-10-men-9004270)
- [John Eustace makes Derby County demand after West Brom defeat as David Ozoh decision explained (NewsBeep)](https://www.newsbeep.com/725829/)
- [Derby County team news and predicted XI to face West Brom (The72)](https://the72.co.uk/2026/09/08/derby-county-team-news-predicted-xi-west-brom/)
- [Derby County vs West Bromwich Albion live score, H2H and lineups (Sofascore, via search)](https://www.sofascore.com/football/match/derby-county-west-bromwich-albion/isC)
